...
class BaseBoostedCascade(BaseGradientBoosting):
    ...
    def _fit_stages(
        ...
        for i in range(begin_at_stage, self.n_layers):
            ...
            if self.verbose > 0:
                ...
                if self.loss not  in {"squared_error", "absolute_error", "huber", "quantile"}:
                    if self._loss.n_classes == 2:
    ...
    def _fit_stage(
        ...
        all_ze_staff = Parallel(n_jobs=5,backend="loky")(delayed(fitter)(eid,estimator,restimator,self.n_splits,self.C,self.n_estimators,self.random_state,self.verbose,X_aug, residual,r, residual.flatten(),history_sum.shape[1], sample_weight, loss) for eid in range(self.n_estimators))
        # all_ze_staff = []
