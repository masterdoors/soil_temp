...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    ...
    def probability_to_sample_weight(self, proba, y_train, x_train):
        ...
        corr_proba = proba.mean(axis=1) #np.array([proba[i, y_train[i]] for i in range(len(y_train))])
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        while self.n_layers_ < self.max_layers:
            ...
            if self.adaptive:
                ...
                sample_weight = self.probability_to_sample_weight(X_aug_train_, y, X_middle_train_)
...
class CascadeForestRegressor(BaseCascadeForest, RegressorMixin):
    def __init__(
        ...
        if weighting_function == "linear":
            def linear_wf(corr_proba, proba, y_train, x_train=None):
                max_train = np.abs(corr_proba - y_train).max() + 0.000001
                corr_proba = np.abs((corr_proba - y_train)/ max_train )
                return corr_proba
    ...
    def predict_sampled(self, X):
        ...
        _y = np.zeros((X.shape[0],self.n_outputs_))
        ...
        for layer_idx in range(self.n_layers_):
            ...
            if layer_idx == 0:
                X_aug_test_ = layer.transform(X_test)
                scores = layer.score_full(X_test)
            elif layer_idx < self.n_layers_ - 1:
                binner_ = self._get_binner(layer_idx)
                X_aug_test_ = self._bin_data(
                    binner_, X_aug_test_, is_training_data=False
            ...
            if scores is not None or layer_idx == self.n_layers_ - 1:
               #print(scores.mean(),scores.std())
                ...
                if layer_idx == self.n_layers_ - 1:
                    if self.n_layers_ == 1:
                        bitmap = np.ones(scores.shape,dtype=bool)
                    else:    
                        bitmap = old_bitmap == 0
                else:    
                    bitmap = scores < self.trx
                ...
                if self.use_predictor:
                    if bitmap.sum() > 0:
                        ...
                        predictor = self.buffer_.load_predictor(self.predictor_)
                        _y[bitmap] = predictor.predict(X_middle_test_[bitmap])
                else:
                    if bitmap.sum() > 0:
                        if self.n_layers_ > 1:
