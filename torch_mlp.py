...
#X[ijk]
#sampleXestimatorXfeatures
class MyMaskedULayer(nn.Module):
    def __init__(self, in_features, out_features, channels,dtype=torch.float64):
        super().__init__()
        self.weight = torch.normal(0,1,size=(in_features, channels, out_features),dtype=dtype)
...
class MLPRB:
    ...
    def fit(self,X,y, indexes = None, test_indexes = None, bias = None,sample_weight = None, init_values = None):
        ...
        #do not train HIDDEN->OUTPUT
        for param in self.model.fc2.parameters():
            param.requires_grad = False        

        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.learning_rate_init,eps=1e-07)#,weight_decay=self.alpha)
        ...
        for epoch in range(self.max_iter):
            ...
            for tensors in train_loader:
                ...
                y_batch = y_batch.repeat(1,1,len(indexes)).reshape(y_batch.shape[0],len(indexes),y_batch.shape[1]).to(device=self.device)
                bias_batch = bias_batch.to(device=self.device)
                mask_batch = mask_batch.to(device=self.device)

                optimizer.zero_grad()                         
                output,_ = self.model(X_batch, mask = mask_batch, bias = bias_batch,avg_pass = False)   
                loss = self.criterion(output.squeeze(), y_batch[mask_batch].reshape(y_batch.shape[0],-1,y_batch.shape[2]).double().squeeze())
                #l2_reg_term = self.alpha * torch.sum(torch.abs(self.model.fc2.weight))
                #loss += l2_reg_term
                eloss += loss.item()
            ...
            with torch.no_grad():
                ...
                if eloss / steps < best_loss:
                    best_model = copy.deepcopy(self.model)    
                    best_loss = eloss / steps
                    last_up = epoch
        ...
        if bias is not None:
            ...
            with torch.no_grad():
                ...
                for tensors in loader:
                    ...
                    output, hidden = self.model(X_batch, mask = mask_batch, bias = bias_batch, avg_pass = True)  
