...
class MaskedPerceptron(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, channels, init_values = None):    
...
class MLPRB:
    ...
    def fit(self,X,y, indexes = None, test_indexes = None, bias = None,sample_weight = None, init_values = None):
        ...
        self.model = MaskedPerceptron(X.shape[2],self.hidden_size,y.shape[1],X.shape[1], init_values=init_values)
        ...
        for epoch in range(self.max_iter):
            ...
            with torch.no_grad():
                for tensors in val_loader:   
                    ...
                    output,_ = self.model(X_batch, mask = mask_batch, bias = bias_batch,avg_pass = True)   
        ...
        if self.verbose:
            print("Stop INPUT->HIDDEN...")
        self.model = best_model    

        #do not train INPUT-> HIDDEN
